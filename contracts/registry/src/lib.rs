...
//!
//! Superseded module slots can be marked deprecated (admin or org owner). Once
//! flagged, [`RegistryInterface::lookup`] routes clients to the designated
//! successor via [`Self::set_migration_target`] when one exists, otherwise it
//! rejects with [`Error::ModuleDeprecated`] — the raw address stays readable
//! through [`Self::get_module_address`] / [`Self::get_module`] for legacy
//! migrations. Deprecated slots reject new `register_module` bindings until
...
impl RegistryContract {
    ...
    ///
    /// Rejects new bindings (`Error::InvalidState`) if the module slot has been
    /// marked deprecated — a retired slot must be reactivated (or removed and
    /// re-registered) before it can be re-bound, so a stale registration can
    /// never silently resurrect a deprecated module.
    ...
    ///
    /// Emits on-chain events for deprecation and blocks future `register_module`
    /// bindings. Once flagged, [`RegistryInterface::lookup`] returns the
    /// migration target if one has been set, else rejects with
    /// [`Error::ModuleDeprecated`]; the raw address remains readable via
    /// [`Self::get_module_address`] / [`Self::get_module`].
    pub fn deprecate_module(
        ...
        astroid_shared::events::publish(
            ...
            ContractEvent::RegistryModuleUpdated {
                org: org.clone(),
                kind,
                address: env.storage().persistent().get::<_, Address>(&mkey).unwrap(),
            },
        );
        env.events().publish(
    ...
    pub fn reactivate_module(
        ...
        Self::bump(&env, &dkey);
        // A reactivated module is no longer deprecated, so a stale migration
        // pointer must not linger in the record.
    ...
    pub fn remove_module(
        ...
        // Clean up deprecation metadata together with the record so a later
    ...
    /// owner. Used for deprecation actions, which are deliberately kept at the
    /// owner/admin level rather than delegated.
...
impl RegistryInterface for RegistryContract {
    fn lookup(env: Env, org: String, kind: ModuleKind) -> Result<Address, Error> {
        Self::check_frozen(&env)?;
        let key = DataKey::Module(org.clone(), kind);
        // Deprecated modules are routed to their designated successor when one
        // has been set; otherwise new interactions are rejected so clients
        // never silently hit a retired implementation.
        if env
            .storage()
            .persistent()
            .get::<_, bool>(&DataKey::ModuleDeprecated(org.clone(), kind))
