...
impl BudgetContract {
    ...
    fn window_transition(
        ...
        // Capacity for the period(s) that just closed, composed of the base
        // limit plus any rollover credit carried in from an earlier surplus.
        ...
        if spent > capacity && budget.allow_deficit {
            ...
            // forward (it reduces the next period's effective limit), and drop
            // any surplus rollover — there is nothing left over to roll.
            ...
            // Surplus: the unspent portion rolls forward, plus one full base
            // limit for every further period that went by entirely untouched.
            ...
            // Rollover disabled: whatever is unspent is simply dropped.
            budget.rollover_credit = 0;
        }
        ...
        if publish {
            let action = if spent > capacity && budget.allow_deficit {
            ...
            let amount = if spent > capacity && budget.allow_deficit {
