...
//!
//! Governance actions (`add_signer`, `remove_signer`, `set_signer_weight`,
//! `set_threshold`, `set_emergency_lock`) are themselves gated: they require the
//! caller to be a current signer and are authorized directly. In production
//! these would typically be routed through the proposal flow too; they are
//! exposed directly here for the platform's administrative bootstrap and kept
//! signer-gated.
//!
...
impl MultiSigContract {
    ...
    pub fn get_signers(env: Env) -> Result<Vec<SignerWeight>, Error> {
        Self::signers(&env)
    }
