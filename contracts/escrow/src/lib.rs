...
impl EscrowContract {
    ...
    /// Close a settled escrow (terminal). Callable once the funds have actually
    /// moved — i.e. from `Released`, `Refunded` or `Cancelled`. An `Expired`
    /// escrow must be `refund`ed, `reclaim`ed or `cancel`led first so custody is
